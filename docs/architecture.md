...
| AI 能力 | `services/ai.service.js` | 文本生成、图片生成、审核、评分、兜底 |
...
| `articles` | `user_id`、`title`、`content`、`media_urls`、`status`、`quality_score`、`view_count` | 草稿和已发布内容 |
...
3. 用户可调用 `/api/v1/ai/generate-image` 生成封面图。
...
- Prompt 管理目前是前端静态模板，不是独立后台 CRUD。
- 素材管理只支持 URL，不支持真实文件上传。
